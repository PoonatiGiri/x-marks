...
async function isRedditConnected() {
  ...
  return !!cookieStore.get("reddit_session")?.value
}

export default async function Home({
  searchParams,
}: {
  searchParams: Promise<{ reddit?: string }>
}) {
  const session = await auth()
  if (session) redirect("/canvas")

  const params = await searchParams
  const redditStatus = params.reddit // "connected" | "denied" | "error" | undefined
  ...
  return (
    <main className="min-h-screen bg-white flex flex-col items-center justify-center px-4">
      <div className="max-w-lg w-full text-center">
        ...
        {/* Reddit pre-connect status banner */}
        {redditStatus === "connected" && (
          <div className="mb-5 flex items-center justify-center gap-2 text-sm text-green-700 bg-green-50 border border-green-100 rounded-xl px-4 py-3">
            <svg className="w-4 h-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
              <path d="M20 6L9 17l-5-5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            Reddit connected. Now sign in with X to open your canvas.
          </div>
        )}
        {redditStatus === "denied" && (
        ...
        <div className="flex items-center gap-3 my-4">
          ...
          <span className="text-xs text-gray-300">also</span>
          <div className="flex-1 h-px bg-gray-100" />
        </div>

        {/* Reddit connect button */}
        {redditConnected ? (
          <div className="w-full flex items-center justify-center gap-2 bg-green-50 border border-green-100 text-green-700 font-medium py-3.5 px-6 rounded-2xl text-sm">
            <svg className="w-4 h-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
              <path d="M20 6L9 17l-5-5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            Reddit connected
